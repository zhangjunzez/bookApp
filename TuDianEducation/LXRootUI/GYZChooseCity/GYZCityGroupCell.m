...
- (UILabel *) titleLabel
{
    if (_titleLabel == nil) {
        ...
        [_titleLabel setFont:[UIFont systemFontOfSize:ScaleW(28)]];
    }
    return _titleLabel;
}
...
- (UILabel *) noDataLabel
{
    if (_noDataLabel == nil) {
        ...
        [_noDataLabel setTextColor:kMainTxtColor];
        [_noDataLabel setFont:[UIFont systemFontOfSize:ScaleW(26)]];
    }
    return _noDataLabel;
}
...
{
    if (isCurrent) {
        if (self.arrayCityButtons.count != 0) {
            UIButton *button = [self.arrayCityButtons objectAtIndex:0];
            [button setBackgroundColor:kBlueColor];
            [button setTitleColor:kWhiteColor forState:UIControlStateNormal];
            [button.layer setMasksToBounds:YES];
            [button.layer setCornerRadius:2.0f];
            //[button.layer setBorderColor:mainClocor.CGColor];
        }
    }
}
...
- (void) layoutSubviews
{
    ...
    self.leftImg.frame = CGRectMake(ScaleW(20), 2, ScaleW(42), ScaleW(43));
    
    float x = WIDTH_LEFT;
    float y = 5;
    CGSize size = [self.titleLabel sizeThatFits:CGSizeMake(MAXFLOAT, MAXFLOAT)];
    [self.titleLabel setFrame:CGRectMake(self.leftImg.right+ScaleW(10), y, self.frame.size.width - x, size.height)];
    y += size.height + 3;
    [self.noDataLabel setFrame:CGRectMake(self.leftImg.right+ScaleW(10) + 5, y, self.frame.size.width - x - 5, self.titleLabel.frame.size.height)];
