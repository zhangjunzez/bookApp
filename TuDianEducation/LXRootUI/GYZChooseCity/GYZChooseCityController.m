...
@property (nonatomic,strong) LXSearchSectionView *headerView;
...
-(void)viewDidLoad{
    ...
    self.isSearch = NO;
//    [self locationStart];
   // [self.popButton addTarget:self action:@selector(popVCAction) forControlEvents:UIControlEventTouchUpInside];
    [self collectListReq];
    self.searchBar = [[UISearchBar alloc]initWithFrame:CGRectMake(0, 0, ScaleW(200), ScaleW(30))];
//    self.searchBar.barStyle     = UIBarStyleDefault;
    self.searchBar.translucent  = YES;
    self.searchBar.delegate     = self;
    self.searchBar.placeholder  = @"请输入城市名";
    self.searchBar.keyboardType = UIKeyboardTypeDefault;
    [self.searchBar setBarTintColor:kWhiteColor];
    
    [self.searchBar setTintColor:kMainTxtColor];
    
//    [self.searchBar.layer setBorderWidth:0.5f];
//    [self.searchBar.layer setBorderColor:kMainTxtColor.CGColor];
    [self.searchBar setCornerRadius:self.searchBar.height/2.f];
    
    //设置背景色为白色
    self.searchBar.backgroundColor = kWhiteColor;
    self.searchBar.backgroundImage=[[UIImage alloc]init];
    if (@available(iOS 13.0, *)){
         self.searchBar.searchTextField.backgroundColor=kWhiteColor;
    }else{
        UITextField*searchField = [self.searchBar valueForKey:@"_searchField"];
        searchField.backgroundColor=kMainBgColor;
    }
    UIView *backView = [[UIView alloc]initWithFrame:CGRectMake(0, 0, ScaleW(200), ScaleW(30))];
    ...
    if(!(searchField == nil)) {
        searchField.textColor = kMainTxtColor;
    ...
    self.tableView.frame = CGRectMake(0, ScaleW(10), ScreenWidth, ScreenHeight-Height_NavBar- ScaleW(10));
    self.tableView.dataSource = self;
    self.tableView.delegate = self;
    [self.tableView setSectionIndexBackgroundColor:[UIColor clearColor]];
    [self.tableView setSectionIndexColor:kGrayTxtColor];
    ...
    [self changeIndexColor];
}

-(LXSearchSectionView *)headerView{
    if (!_headerView) {
        _headerView = [[LXSearchSectionView alloc]init];
        WS(weakSelf);

        _headerView.locationBlock = ^{
            [weakSelf locationStart];
        };
        _headerView.locationBlock = ^{
            [weakSelf locationStart];
        };
...
-(void)dataWithArray:(NSArray *)dataArray
{
           ...
           for (NSDictionary *groupDic in array) {
               ...
               for (NSDictionary *dic in [groupDic objectForKey:@"citys"]) {
                   ...
                   for (NSDictionary *dic  in dataArray) {
                       if ([dic[@"name"] isEqualToString:@"郑州市"]) {
                           [LXSaveUserInforTool sharedUserTool].cityId = dic[@"cid"];
                       }
                       if ([dic[@"name"] isEqualToString:city.cityName]) {
                           city.cid = dic[@"cid"];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (self.isSearch) {
        ...
        cell.textLabel.font = [UIFont systemFontOfSize:ScaleW(12)];
        return cell;
    }
    
//    if (indexPath.section < 2) {
//        GYZCityGroupCell *cell = [tableView cellForRowAtIndexPath:indexPath];
//        if (!cell) {
//            cell = [[GYZCityGroupCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cityGroupCell];
//        }
//        if (indexPath.section == 0) {
//            cell.leftImg.image = [UIImage imageNamed:@"定位(1)"];
//            cell.titleLabel.text = @"当前城市";
//            cell.noDataLabel.text = @"";
////            [cell setCityArray:self.localCityData];
//            GYZCity *city = [[GYZCity alloc] init];
//            city.cityName = self.currentCity;
//            [cell setCityArray:@[city]];
//            cell.isCurrent = YES;
//        }else {
//            cell.leftImg.image = [UIImage imageNamed:@"城市"];
...
//    }
    UITableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:cityCell];
    //GYZCityGroup *group = [self.cityDatas objectAtIndex:indexPath.section];
   // GYZCity *city =  [group.arrayCitys objectAtIndex:indexPath.row];
    GYZCity *city =  [self.dataArray objectAtIndex:indexPath.row];
    [cell.textLabel setText:city.cityName];
    cell.textLabel.textColor = kMainTxtColor;
    cell.textLabel.font = [UIFont systemFontOfSize:ScaleW(12)];
...
- (UIView *) tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section
{
    ...
    headerView.contentView.backgroundColor = kMainBgColor;
    return headerView;
}
...
//        }
    ...
    city = self.dataArray[indexPath.row];
...
-(void)searchBarTextDidBeginEditing:(UISearchBar *)searchBar{
    ...
    UIButton *cancelButton = (UIButton *)[self findViewWithClassName:NSStringFromClass([UIButton class]) inView:searchBar];
    ...
    [geocoder reverseGeocodeLocation:currentLocation completionHandler:^(NSArray *array, NSError *error)
     {
         if (array.count >0)
         {
             ...
             if (self.localCityData.count <= 0) {
                 ...
                 CGFloat cityWidth = [self.view returnWidth:currCity  font:ScaleW(14)];
                 [self.headerView.loactionBtn setTitle:currCity forState:(UIControlStateNormal)];
                 self.headerView.loactionBtn.width = cityWidth +ScaleW(30);
                 //[self.tableView reloadData];
             }
...
-(void)changeIndexColor{
    ...
    self.tableView.sectionIndexBackgroundColor = [UIColor clearColor];
       ...
       self.tableView.sectionIndexColor = kGrayTxtColor;
