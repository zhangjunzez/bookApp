//
//  LXEdtingServerViewController.m
//  TuDianEducation
//
...
@interface LXEdtingServerViewController ()<UITextViewDelegate,GYZChooseCityDelegate>
...
@implementation LXEdtingServerViewController
...
-(LXTitleImgeArrayItemView *)lunboView{
    if (!_lunboView) {
        _lunboView = [[LXTitleImgeArrayItemView alloc]initWithTop:ScaleW(10) title:@"轮播图片" imgeArray:@[@""] redHidden:NO];
...
-(void)commitAction:(UIButton *)sender{
    if ([self pamasIsOkOrNot]) {
        NSDictionary *pamas = @{@"servicesid":_model.servicesid, @"cid":self.cityId,@"inid":_model.inid,@"name":_severNameView.textFied.text,@"title":_fubiaotiView.textFied.text,@"price":_fuwuPriceView.textFied.text,@"introduction":_serverjianjieView.textView.text,@"content":_xiangqingView.textView.text,@"bannerimages":_lunboView.urlsArray,@"detailimages":_detailPicView.urlsArray};
        WS(weakSelf);
...
-(void)setDefultValue{
    ...
    self.lunboView.setServerArray = _model.bannerimages;
    self.hangyeFenView.textFied.text = _model.industrys;
    self.severNameView.textFied.text = _model.servicesname;
    self.fubiaotiView.textFied.text = _model.servicestitle;
    self.fuwuPriceView.textFied.text = _model.servicesprice;
    self.serverjianjieView.textView.text = _model.servicesintroduction;
    self.serverjianjieView.placeHolderLabel.hidden = YES;
    self.xiangqingView.textView.text = _model.servicescontent;
    self.xiangqingView.placeHolderLabel.hidden = YES;
    self.detailPicView.setServerArray = _model.detailimages;
    self.currentId = _model.inid;
    self.cityId = _model.cid;
    self.cityView.textFied.text = _model.cityname;
//    NSString *name = @"";
