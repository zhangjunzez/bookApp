...
@interface LXTuijianControlView()<UIScrollViewDelegate>
...
@implementation LXTuijianControlView
...
{
    if (!_bottomLine) {
        _bottomLine = [[UIView alloc]initWithFrame:CGRectMake(0, self.height - ScaleW(1), ScreenWidth, ScaleW(1))];
...
-(void)setBttonsArray:(NSArray *)bttonsArray
{
    ...
    for (int i = 0; i < _bttonsArray.count; i ++) {
        CGFloat titleWidth = ScreenWidth/2.f;
        UIButton *btn = [UIButton buttonWithType:UIButtonTypeCustom];
        btn.frame = CGRectMake( allWidth, 0, (titleWidth), ScaleW(40));
        [self.contenScrollView addSubview:btn];
        btn.tag = kTag(i);
        [btn btn:btn font:ScaleW(16) textColor:kMainTxtColor text:SSKJLocalized(_bttonsArray[i], nil) image:nil sel:@selector(bttonsAction:) taget:self];
